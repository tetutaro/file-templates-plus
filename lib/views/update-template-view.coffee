...
module.exports =
	class UpdateTemplateView extends View
		...
		initialize: (template) ->
			...
			@extEditor.setText(template.ext)
		...
		updateTemplate: (event, element) ->
			...
			@template.ext = @extEditor.getText()
		...
		editContents: (event, element) ->
			@destroy()
			item = @template
			atom.workspace.open(path.join(atom.config.get('file-templates-plus.templateStore'), item.hash + '.template')).then ->
